...
try:
    if sys.platform != 'win32':
        import uvloop
        uvloop.install()
        logger.info("[Grok for Learning Only] uvloop high-performance event loop enabled")
    else:
        logger.info("[Grok for Learning Only] Windows system, using default asyncio event loop")
except ImportError:
    logger.info("[Grok for Learning Only] uvloop not installed, using default asyncio event loop")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("[Grok for Learning Only] Core services initialized")
    ...
    # 3. 异步Loading token 数据
    await token_manager._load_data()
    logger.info("[Grok for Learning Only] Token data loaded")
    ...
    logger.info("[Grok for Learning Only] Application started successfully")
    ...
    try:
        ...
        logger.info("[Grok for Learning Only] Application shutdown complete")


# 初始化日志
logger.info("[Grok for Learning Only] Application starting...")

# 创建FastAPI应用
app = FastAPI(
    title="Grok for Learning Only",
...
if __name__ == "__main__":
    ...
    if workers > 1:
        logger.info(
            f"[Grok for Learning Only] 多进程模式已启用 (workers={workers})。"
