...
class CacheService:
    ...
    async def download(self, file_path: str, auth_token: str, timeout: Optional[float] = None) -> Optional[Path]:
        ...
        for outer_retry in range(MAX_OUTER_RETRY + 1):  # +1 确保实际重试3 times
            try:
                # 内层重试：403代理池重试（cache使用缓存代理，不支持代理池，所以403只重试一 times）
                max_403_retries = 5
                retry_403_count = 0
                ...
                while retry_403_count <= max_403_retries:
                    ...
                    async with AsyncSession() as session:
                        ...
                        # 检查403errors - 内层重试(cache不使用代理池，所以直接Failed)
                ...
                if outer_retry < MAX_OUTER_RETRY - 1:
                    self._log("warning", f"下载Exception: {e}，outer retry ({outer_retry+1}/{MAX_OUTER_RETRY})...")
                    await asyncio.sleep(0.5)
                    continue
                
                self._log("error", f"Download failed: {e}（已重试{outer_retry} times）")
                return None
    ...
    async def _safe_cleanup(self):
        ...
        try:
            ...
            self._log("error", f"后台Cleanup failed: {e}")
...
class ImageCache(CacheService):
    ...
    async def download_base64(self, path: str, token: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"[ImageCache] Failed to download base64: {e}")
            return None
